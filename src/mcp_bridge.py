        ...
        async def handle_call_tool(name: str, arguments: dict | None) -> list[types.TextContent]:
            try:
                async with httpx.AsyncClient(timeout=30.0) as client:
                    response = await client.post(f"{self.server_url}/tools/{name}", json=arguments or {})
                    data = response.json()
        ...
        if tool_name == "search_foods":
            ...
            if macros:
                ...
                for nutrient, info in macros.items():
                    result += f"• {nutrient}: {info.get('amount', 0)}{info.get('unit', '')}\n"
                result += "\n"
    ...
    async def run(self):
        ...
        try:
            ...
            if "localhost" in self.server_url:
                ...
                print("   docker run -p 8080:8080 -e USDA_API_KEY=your_key nutrition-mcp")
            else:
